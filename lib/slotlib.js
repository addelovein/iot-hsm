    ...
    if(idindex <= ids.length - 1) {
        ...
        if(typeindex <= types.length - 1) {
            ...
            if(objectindex <= objects.length - 1) {
                ...
                if(type) {
                    // console.log({slotid: slot.hexid, objectid: slot.objects[ids[idindex]][types[typeindex]][objects[objectindex]].ID, type: type, modulePath: slot.modulePath});
...
var getCertificateObjects = function(slots, index, callback) {
    if(index===null || index===false) {
        index = 0;
    }
    if(index <= slots.length - 1) {
        //if(slots[index].hasOwnProperty('token state') && slots[index]['token state'] == 'uninitialized') {
        //    getCertificateObjects(slots, index + 1, callback);
        //} else {
            openssl2.pkcs11.listObjects({modulePath: slots[index].modulePath, slotid: slots[index].hexid}, function(err, objects) {
                ...
                if(err) {
                    ...
                    for(let i = 0; i <= objects.data.length - 1; i++) {
                        ...
                        // if google HSM
                        } else if(objects.data[i].type == 'Private Key Object' && objects.data[i].ID.length > 100) {
                            slots[index].objects[objects.data[i].label] = {};
                            let certs = statelib.get().certificates;
                            // console.log(slots[index]);
                            // console.log(certs);
                            //if found in cache, add to slot object
                            if(certs && certs.hasOwnProperty(slots[index]['serial num']) && certs[slots[index]['serial num']].hasOwnProperty(objects.data[i].label)) {
                                slots[index].objects[objects.data[i].label] = certs[slots[index]['serial num']][objects.data[i].label];
                                // add private key object info so we can get properties like "allowed mechanisms"
                                slots[index].objects[objects.data[i].label]['Certificate Object'][0]['private_key'] = objects.data[i];
                                // console.log(slots[index].objects[objects.data[i].label]);
                            }
                            // slots[index].objects[objects.data[i].label]['Certificate Object'] = [];
                            // console.log(slots[index]);
                            // console.log(statelib.get().certificates);
                        }
                    }
                    // console.log(slots[index]);
...
var getSlots = function(slots, modules, index, callback) {
    if(index===null || index===false) {
        index = 0;
    }
    if(index <= modules.length - 1) {
        openssl2.pkcs11.listSlots({modulePath: modules[index]}, function(err, pkcs11slots) {
            if(err) {
                ...
                getSlots(slots, modules, index + 1, callback);
            } else {
...
var deleteObject = function(params, callback) {
    // console.log(params);
    let moduletype = params.module.split('/').at(-1);
    if(moduletype == 'libkmsp11.so') {
        statelib.deleteCertificate(params, cachedslots, function(err, state) {
            if(err) {
                callback(err, false);
            } else {
    ...
    } else if(moduletype == 'libykcs11.so') {
        let yubikeySlot = mapping[params.objectid];
        if(!yubikeySlot) {
            callback('Unsupported YubiKey slot ID', false);
            return;
        }
        let mgmtkey = params.mgmtkey || params.pin || config.SOPIN;
        let action;
        if(params.type=='Certificate Object') {
            action = 'delete-certificate';
        } else if(params.type=='Public Key Object' || params.type=='Private Key Object') {
            action = 'delete';
        } else {
            callback('Invalid object type', false);
            return;
        }
        let cmd = ['-a' + action + ' -s ' + yubikeySlot + ' --key=' + mgmtkey];
        yubicopivtool.run({cmd: cmd.join(' ')}, function(err, out) {
            if(err) {
                callback(err, false);
            } else {
                cachedslots.slots = [];
...
var generateKeyPair = function(params, callback) {
    ...
    let label = prepLabel(params.label);
    let signpin = params.signpin || params.pin;
    let keygen = function(done) {
    ...
    let ykKeygen = function(done) {
        let yubikeySlot = mapping[params.objectid];
        if(!yubikeySlot) {
            done('Unsupported YubiKey slot ID', false);
            return;
        }
        let algo;
        if(params.keytype == 'rsa:2048') {
            algo = 'RSA2048';
        } else if(params.keytype == 'EC:prime256v1') {
            algo = 'ECCP256';
        } else if(params.keytype == 'EC:secp384r1') {
            algo = 'ECCP384';
        } else if(params.keytype == 'rsa:4096') {
            done('YubiKey PIV does not support RSA 4096', false);
            return;
        } else {
            done('Unsupported YubiKey key type', false);
            return;
        }
        let mgmtkey = params.mgmtkey || config.SOPIN;
        tmp.file(function _tempFileCreated(err, pubkeypath, fd, cleanupCallback) {
            if (err) {
                cleanupCallback();
                done(err, false);
            } else {
                let cmd = ['-agenerate -s ' + yubikeySlot + ' -A ' + algo + ' --key=' + mgmtkey + ' -o ' + pubkeypath];
                yubicopivtool.run({cmd: cmd.join(' ')}, function(err, out) {
                    cleanupCallback();
                    if(err) {
                        done(err, false);
    ...
    let finalize = function() {
        ...
        let mup = common.moduleURIParam(params.module);
        openssl2.x509.selfSignCSR({
                options: csroptions,
                pkcs11: {
                    uri: openssl2.common.encodePKCS11URI({
                        serial: params.serial,
                        [mup.param]: mup.prefix + params.objectid,
                        type: 'private'
                    }),
                    pin: signpin,
    ...
    if(params.module.indexOf('libykcs11.so') >= 0) {
        ykKeygen(function(err) {
            if(err) {
                callback(err, false);
            } else {
                finalize();
...
var generateSelfSigned = function(params, callback) {
    // console.log(params);
    // console.log(JSON.stringify(cachedslots,null,2));
    openssl2.pkcs11.listObjects({modulePath: params.module, slotid: params.slotid}, function(err, objects) {
        if(err) {
            callback(err, false);
        } else {
            ...
            for(let i = 0; i <= objects.data.length - 1; i++) {
                // console.log(objects.data[i]);
                if(objects.data[i].type == 'Private Key Object' && objects.data[i].label == params.objectid) {
                    hash = common.getAllowedHashAlg(objects.data[i]);
                    break;
                }
            }
            // console.log(hash);
            getCertInfo(params, function(err, cert) {
                if(err) {
                    ...
                    // console.log(params.module.split('/').at(-1));
                    let mup = common.moduleURIParam(params.module);
                    openssl2.x509.selfSignCSR({
                            options: csroptions,
                            pkcs11: {
...
var signCSR = function(params, callback) {
    // console.log(params);
    let hashalg = params.hash;
    let state = statelib.get();
    if(state.hasOwnProperty('certificates') && state.certificates.hasOwnProperty(params.serial)) {
        if(state.certificates[params.serial].hasOwnProperty(params.objectid)) {
            if(state.certificates[params.serial][params.objectid].hasOwnProperty('Certificate Object')) {
                if(state.certificates[params.serial][params.objectid]['Certificate Object'][0].hasOwnProperty('private_key')) {
                    hashalg = common.getAllowedHashAlg(state.certificates[params.serial][params.objectid]['Certificate Object'][0].private_key);
                    // console.log(statecerts[params.serial][params.objectid]['Certificate Object'][0].private_key);
                }
            }
        }
    }
...
var importCertificate = function(params, callback) {
    ...
    // import Google HSM cert
    if(params.module.split('/').at(-1)=='libkmsp11.so') {
        // console.log(params);
        statelib.addCertificate({serial: params.serial, objectid:params.objectid, cert: params.cert}, function(err, state) {
            if(err) {
                console.log(err);
            } else {
                // console.log(state);
                // console.log(statelib.get());
                cachedslots.slots = []
                callback(false, {});
            }
        });
    } else if(params.module.indexOf('libykcs11.so') >= 0) {
        let yubikeySlot = mapping[params.objectid];
        if(!yubikeySlot) {
            callback('Unsupported YubiKey slot ID', false);
            return;
        ...
        tmp.file(function _tempFileCreated(err, objectpath, fd, cleanupCallback) {
            if (err) {
                cleanupCallback();
                callback(err, false);
            } else {
                fs.writeFile(objectpath, cert, function() {
                    if(err) {
                        cleanupCallback();
                        callback(err, false);
                    } else {
                        let cmd = ['-aimport-certificate -s ' + yubikeySlot + ' --key=' + mgmtkey + ' -i ' + objectpath];
                        yubicopivtool.run({cmd: cmd.join(' ')}, function(err, out) {
                            cleanupCallback();
                            if(err) {
                                callback(err, false);
...
var getModules = function(callback) {
    // update modules to be dynamic
    var modules = [
        config.LIB + '/softhsm/libsofthsm2.so',
        config.LIB + '/x86_64-linux-gnu/libykcs11.so'
    ]

    // don't require Google creds to load Google KMS module because the user may be using a different method of authentication
    fs.stat(config.GOOGLE_APPLICATION_CREDENTIALS, function(err, stats) {
        if(err) {
            // don't load google hsm
        } else {
            process.env['GOOGLE_APPLICATION_CREDENTIALS'] = config.GOOGLE_APPLICATION_CREDENTIALS;
        }
        fs.stat(config.KMS_PKCS11_CONFIG, function(err, stats) {
            if(err) {
                // don't load google hsm
            } else {
                modules.push(config.LIB + '/kms/libkmsp11.so');
            }
            ...
            if(force || cachedslots.slots.length == 0) {
                ...
                getModules(function(err, modules) {
                    if(err) {
                        ...
                        getSlots([], modules, false, function(err, slots) {
                            //console.log(slots);
                            ...
                            if(err) {
                                ...
                                getCertificateObjects(slots, false, function(err) {
                                    if(err) {
                                        cachedslots.state = 'error';
                                        console.log(err);
                                    } else {
